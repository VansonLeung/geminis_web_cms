...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var inIcon = '<span class="fc-title">'+(htmlEscape(event.title || '') || '&nbsp;') +'</span>';
		var outIcon = '';
		if(event.ipotype=='pre' && event.end != null){
			if(seg.isStart == true && seg.isEnd == true){
				inIcon = '<td width="50%"><img src="../images/ipo/ipo_calendar_start.png" style="height:18px; margin-right:5px;" />' + (htmlEscape(event.title || '') || '&nbsp;') +'</td>';
				outIcon = '<td width="50%" style="    text-align: right;"><img src="../images/ipo/ipo_calendar_close.png" style="height:18px; margin-right:5px;" />' + (htmlEscape(event.title || '') || '&nbsp;') +'</td>';
				titleHtml = '<div class="fc-title"><table><td>'+ inIcon + outIcon  +  '</tr></table></div>';
			}else{
				if(seg.isStart == true){
				inIcon = '<img src="../images/ipo/ipo_calendar_start.png" style="height:18px; margin-right:5px;" />' + (htmlEscape(event.title || '') || '&nbsp;');
				outIcon = '';
				titleHtml = '<span class="fc-title">'+ inIcon + outIcon  +  '</span>';
				}else{
				inIcon = '';
				outIcon = '<img src="../images/ipo/ipo_calendar_close.png" style="height:18px; margin-right:5px;" />' + (htmlEscape(event.title || '') || '&nbsp;');
				titleHtml = '<span class="fc-title" style="float:right">'+ inIcon + outIcon  +  '</span>';
				}
			}
		}else{
			if(event.ipotype == 'open'){
			titleHtml = '<span class="fc-title"><img src="../images/ipo/ipo_calendar_listing.png" style="height:18px; margin-right:5px;" />'+ inIcon + outIcon  +  '</span>';
			}else{
				titleHtml = '<span class="fc-title">'+ inIcon + outIcon  +  '</span>';
			}
		}
